...
export default function Home() {
   ...
   useEffect(() => {
      if (!songs.length && !searchParams.has('id')) router.push('from-osu/select');
   }, [songs]);
   ...
   useEffect(() => {
      ...
      if (playlistQuery.data?.error && !songs.length && searchParams.has('id')) {
         router.push('from-osu/select');
         return;
      };
      if (playlistQuery.data) {
         setSongs(playlistQuery.data.beatmapsets.map((id: string) => (
