...
export default function FromOsu() {
  ...
  const isLoading = combinedArray.some(
    (q) => q.spotifyQuery.isLoading || q.beatmapsetQuery.isLoading,
  );
  ...
  useEffect(() => {
    ...
    const sortedGroupedArray = groupArray(
      groupFn,
      sortOrder,
      sortFn,
      combinedArray,
    ...
    <div className="overflow-y-hidden max-h-screen min-w-[600px] min-h-[670px]">
      ...
      {isLoading && msLeft > 5000 && (
        <div className="absolute top-1 right-0 z-1000 bg-highlight/30 text-xs px-1 rounded-bl-sm min-w-[120px] text-end">
          {beatmapsetQueries.filter((q) => !q.isLoading).length}/{songs.length}{" "}
          | {timeLeft} left
        </div>
      )}
      ...
      <header className="bg-triangles border-b-4 border-main-border w-screen h-14 flex justify-between items-center px-4 gap-3">
        <section className="flex gap-3 items-center min-w-fit">
          ...
          <Image
            ...
            className={tw(
              "hover:animate-spin hover:duration-2000 cursor-pointer",
              isSettingsVisible && "brightness-130",
            )}
          />
        </section>
        ...
        >
          <label
            className="font-semibold hidden lgx:block"
          ...
          <input
            ...
            id="filter-select"
            onChange={(e) =>
              setFilters(e.target.checked ? ["exact-spotify"] : [])
            }
        ...
        >
          <label
            className="text-md font-semibold tracking-wider hidden xl:block"
        ...
        >
          <label
            className="text-md font-semibold tracking-wider hidden lgx:block"
            htmlFor="sort-select"
          >
            Sort
          </label>
      ...
      <Tooltip
        id="tooltip"
        place="bottom"
        style={{ fontSize: "13px", padding: "0 0.25rem", zIndex: 100000 }}
      />
    </div>
  );
}
