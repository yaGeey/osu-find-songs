...
export default function PLaylistPage() {
  ...
  const [filteredBeatmapsets, setFilteredBeatmapsets] = useState<
    BeatmapSet[][]
  >([]);

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [isModalDownloadingVisible, setIsModalDownloadingVisible] =
    useState(false);
  const [isModalDownloadedVisible, setIsModalDownloadedVisible] =
    useState(false);
  ...
  const {
    ...
    queryFn: async ({
      pageParam = `https://api.spotify.com/v1/playlists/${playlistId}/tracks?offset=0&limit=100`,
    }) => await fetchWithToken(pageParam),
    getNextPageParam: (lastPage) => (lastPage.next ? lastPage.next : undefined),
    getPreviousPageParam: (firstPage) =>
      firstPage.previous ? firstPage.previous : undefined,
  ...
  const tracks =
    tracksData?.pages.map((page: PlaylistPage) => page.items).flat() || [];
  ...
  const beatmapsetQueries = useQueries({
    queries: tracks.map((track) => ({
      queryKey: [
        "beatmapset",
        track.track ? track.track.artists[0].name : "err",
        track.track ? track.track.name : "err",
      ],
      queryFn: async () => {
  ...
  const isLoading = useMemo(
    () => beatmapsetQueries.some((q) => q.isLoading),
    [beatmapsetQueries],
  );
  ...
  function handleDownloadAll(video: boolean) {
    ...
    const promise = zip
      ...
      .then((blob) => download(blob, "beatmaps.zip"));
  ...
  return (
    <div className="max-h-screen min-w-[800px] min-h-[670px] font-inter overflow-y-auto scrollbar">
      ...
      <div
        className={tw(
          "fixed top-0 h-0.6 z-100000 w-screen text-main-lighter",
          timeToSearch ? "block" : "hidden",
        )}
      >
        <LinearProgress
          variant="determinate"
          value={(timeToSearch! * 100) / 2000}
          color="inherit"
        />
      </div>
      <Progress
        isLoading={isLoading}
        value={
          (beatmapsetQueries.filter((q) => !q.isLoading).length * 100) /
          tracks.length
        }
      />
      {isLoading && msLeft > 5000 && (
        <div className="absolute top-1 right-0 z-1000 bg-highlight/30 text-xs px-1 rounded-bl-sm min-w-[120px] text-end">
          {beatmapsetQueries.filter((q) => !q.isLoading).length}/{tracks.length}{" "}
          | {timeLeft} left
        </div>
      )}
      ...
      >
        ...
        <Button
          ...
          className="text-white py-1 w-45"
        >
          Download all
      ...
      <main className="flex justify-center items-center min-h-[calc(100vh-4rem)] mt-[56px]">
        <div className=" min-h-[calc(100vh-3.5rem)] bg-darker [@media(min-width:980px)]:w-4/5 w-full ">
          <Filters
            foundString={
              beatmapsets.length
                ? beatmapsets.filter((a) => !!a && a.length).length +
                  "/" +
                  beatmapsets.length
                : ""
            }
          ...
          <div className="flex p-4 gap-4 flex-wrap bg-darker overflow-y-auto">
            {uniqueBeatmapsetMatrix(
              filteredBeatmapsets.filter((data) => data && data.length),
            )
              .sort((a, b) =>
                sortBeatmapsMatrix(
                  a,
                  b,
                  searchParams.get("sort") || "relevance_asc",
                ),
              )
              .map((data, i) => {
              ...
              !isLoading && (
                <div className="text-black/40 text-2xl h-full w-full text-center mt-10 animate-in fade-in">
                  ...
                  <p className="text-base">
                    Try setting the state to 'any' to see unranked maps
                  </p>
                </div>
              )}
          </div>
      ...
      >
        <p className="text-balance text-center">
          If there is more than one beatmap set for a song, the first one based
          on your search{" "}
          <span className="text-highlight font-outline">filters</span> will be
          downloaded
        </p>
