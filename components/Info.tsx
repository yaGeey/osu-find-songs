...
export default function Info({ data, onClose, className }: Props) {
  ...
  const [selection, setSelection] = useState<"spotify" | "youtube" | "other">(
    "spotify",
  );
  ...
  useEffect(() => {
    if (!Cookies.get("showSpotifyEmbeds"))
      Cookies.set("showSpotifyEmbeds", "true");
    if (!Cookies.get("showYouTubeEmbeds"))
  ...
  const yt = useQuery({
    ...
    queryFn: async (): Promise<Media[]> => {
      ...
      const { data } = await axios.get(
        `/api?query=${encodeURIComponent(song.author + " " + song.title)}`,
      );
      return data;
    },
    ...
    >
      ...
      <div className="flex gap-4 h-[120px]">
        ...
        <div className="flex flex-col justify-between text-ellipsis">
          <div>
            <h1 className="flex items-end gap-1.5 font-outline-sm">
              ...
              {spotify && spotify?.length != 20 && (
                <span className="text-sm font-medium mb-0.5">
                  {spotify[0].album.release_date.split("-")[0]}
                </span>
              )}
            </h1>
            ...
            <h2 className="text-base font-medium mt-1 line-clamp-2 font-outline-sm text-[15px]">
              {spotify?.length != 20 && beatmapset ? (
                <AuthorString
                  artists={spotify ? spotify[0].artists : []}
          ...
          <div className="flex justify-between items-end">
            {spotify &&
              spotify?.length != 20 &&
              spotify[0].album.name != local.title && (
            ...
            {wikiQuery.data?.content && (
              <div className="relative font-outline-sm overflow-hidden h-8 w-4/5 px-1 bg-white/20 rounded-lg flex-grow">
                <p className="text-xs pr-24 text-justify">
                  {wikiQuery.data?.content}
                </p>
      ...
      <div className="flex w-full items-end gap-4 mt-4">
        ...
        <YoutubeBtn
          ...
          className={selection == "youtube" ? "selection" : ""}
        />
        <a
          target="_blank"
          ...
          {spotify?.length == 20 && (
            <div className="flex gap-2">
              ...
              <span className="text-lg font-semibold">
                The song wasn't found through a normal search query, so there
                could be a ton of useless results
              </span>
            </div>
          )}
        ...
        <li className="relative scrollbar flex flex-wrap gap-2 mt-3 bg-[#0909094D] box-border w-full h-full p-2 rounded-lg border-[4px] border-light overflow-auto">
          ...
          {yt.data?.map((media: Media, i: number) => {
            if (Cookies.get("showYouTubeEmbeds") == "true") {
              ...
              if (media.type === "SONG")
                return <YtSongEmbed key={i} song={media} />;
            }
          })}
        </li>
      )}
