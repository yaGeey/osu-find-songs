...
export default function CreatePlaylistButton({
  ...
  function navigateToAuth() {
    ...
    const encodeRedirectUri = encodeURIComponent(
  ...
  async function handleCreatePlaylist() {
    ...
    if (!Cookies.get("spotify_oauth_access_token")) {
      handleModal(
        <h1>You must be logged in your Spotify account to continue!</h1>,
        "warning",
        navigateToAuth,
        "Login",
      );
    ...
    if (songQueries.filter((q) => q.data).length !== songQueries.length) {
      handleModal(
        <h1>Some songs are still loading, try again later</h1>,
        "warning",
      );
      return;
    }
    ...
    handleModal(
      <h1 className="animate-pulse">Fetching profile...</h1>,
      "loading",
    );
    ...
    handleModal(
      <h1 className="animate-pulse">Creating playlist...</h1>,
      "loading",
    );
    ...
    handleModal(
      <h1 className="animate-pulse">Putting tracks in your playlist...</h1>,
      "loading",
    );
  ...
  return (
    <>
      <Button
        ...
        className={tw(
          "bg-main-border/50 text-white py-1.5 md:whitespace-nowrap min-w-[135px]",
          className,
        )}
      >
