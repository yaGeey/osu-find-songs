...
export default function SwitchFull({ options, onChange, disabled = false, required = false, defaultValue, className }: {
      ...
      >
         ...
         <div className="bg-darker/80 text white h-full p-1 rounded-full flex items-center gap-2.5 px-2.5 select-none">
            {Object.entries(options).map(([key, val], index) => (
               <button
                  key={index}
                  className={tw("cursor-pointer mb-0.5 text-sm hover:selected transition-all", isDisabled && 'pointer-events-none', selection == val && 'selected')}
