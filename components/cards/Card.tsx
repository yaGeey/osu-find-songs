  ...
  switch (sortFn) {
    ...
    case "sort-date-mapped":
      return data.beatmapsetQuery.data?.submitted_date
        ? new Date(
            data.beatmapsetQuery.data.submitted_date,
          ).toLocaleDateString()
        : null;
    ...
    case "sort-length":
      return data.beatmapsetQuery.data?.beatmaps[0].total_length
        ? new Date(data.beatmapsetQuery.data?.beatmaps[0].total_length * 1000)
            .toISOString()
            .slice(14, 19)
        : null;
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(local.image || beatmapsetQuery.data?.covers.card) && (
          <div className="relative w-[150px] h-[85.5px]">
            <Image
              src={local.image || beatmapsetQuery.data?.covers.card || ""}
              alt={local.title || "alt"}
              fill
        ...
        <div className="flex justify-between items-center w-full py-2 px-4">
          <div className="w-fit max-w-[250px] ">
            <h1 className="text-lg font-outline-sm">
              {local.title || beatmapsetQuery.data?.title}
            </h1>
            <h2 className="text-white/85 text-sm font-outline-sm">
              {local.author || beatmapsetQuery.data?.artist}
            </h2>
          </div>
          <div
            className={tw(
            ...
            {beatmapsetQuery.isLoading && !spotifyQuery.isLoading && (
              <div className="relative w-[30px] h-[30px] flex justify-center items-center">
                <Image
                  ...
                  height={30}
                  alt="osu"
                  className="animation-in fade-in blur-[1px]"
                />
                <Spinner
                  radius={33}
                  color="#ff87c6"
            ...
            {spotifyQuery.data && spotifyQuery.data?.length == 1 && (
              ...
              >
                <Image
                  ...
                  height={30}
                  alt="Spotify"
                  className="animation-in fade-in"
                />
              </a>
            )}
            {spotifyQuery.data &&
              spotifyQuery.data?.length > 1 &&
              spotifyQuery.data?.length !== 20 && (
