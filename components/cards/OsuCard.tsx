...
export default function OsuCard({ beatmapset, className }: { beatmapset: BeatmapSet, className?: string }) {
   return (
      <>
         <div className={tw("group select-none relative h-26 font-inter rounded-2xl min-w-[386px] w-[464px] bg-main flex border-2 border-main-border hover:brightness-110 transition-all", className)}>
            <div className="relative w-[100px] h-full rounded-l-2xl overflow-hidden z-0">
               <Image src={beatmapset.covers.list} alt="list" fill style={{ objectFit: 'cover' }} />
            </div>
            <div className="relative flex-grow flex justify-end rounded-r-2xl overflow-hidden">
               <Image src={beatmapset.covers.card} alt="cover" width={286} height={100} className="z-10"/>
               <div className="absolute top-0 right-0 w-[288px] h-full bg-gradient-to-r from-main to-main/70 rounded-r-2xl z-15"></div>
            </div>
            ...
            {/* download buttons */}
            <div className="absolute top-0 right-0 h-full w-7 bg-darker hidden flex-col items-center justify-center gap-5  text-black/50 text-sm group-hover:flex z-100 rounded-r-[14px] overflow-hidden">
               <FontAwesomeIcon
                  icon={faDownload}
         ...
         <Tooltip id='tooltip' place="top" style={{fontSize: '11px', padding: '0 0.25rem'}}/>
      </>
   )
}