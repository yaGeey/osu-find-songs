...
const sortFn = (sortQuery: string) => (a: BeatmapSet, b: BeatmapSet) => {
  ...
  if (sort === "ranked") {
    ...
    return (
      sign *
      (new Date(a.ranked_date).getTime() - new Date(b.ranked_date).getTime())
    );
  }
  ...
  return (
    <>
      <div
        ref={ref}
        className={tw(
          "relative h-[120px] group min-w-[386px] w-[464px] cursor-pointer",
          className,
        )}
      ...
      {isDialogOpen &&
        ReactDom.createPortal(
          <div
            className={`fixed top-0 left-0 w-screen h-screen bg-black/40 z-1000`}
          >
            <div
              className={
