...
export default function SpotifyEmbed({ track }: { track: SpotifyTrack }) {
   return (
      <a
      ...
      >
         ...
         <div className="w-4/5 pl-4 flex flex-col p-1 font-outline-sm">
            ...
            <p className="text-sm">{track.artists.items.map((a) => a.profile.name).join(', ')}</p>
         ...
         <div className="flex flex-col items-end justify-between p-2 h-full ">
            ...
            <p className="bg-main-border/70 rounded-xl py-0.5 px-1.5 text-xs">
               {new Date(track.duration.totalMilliseconds).toISOString().slice(14, 19)}
            </p>
         </div>
      </a>
   )
