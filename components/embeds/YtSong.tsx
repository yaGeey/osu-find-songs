...
export default function YtSongEmbed({ song }: { song: Song }) {
   ...
   return (
      ...
      >
         ...
         />
         <div className="w-4/5 pl-4 flex flex-col p-1 justify-evenly gap-1 font-outline-sm">
            <h2 className="text-base font-semibold">{song.name}</h2>
            <p className="text-sm">{song.artist.name}</p>
         </div>
         {song.duration && <div className="flex flex-col items-end justify-end p-2 h-full">
