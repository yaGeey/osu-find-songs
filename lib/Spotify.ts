...
export const searchSongWithConditions = async (
  ...
  for (const condition of conditions) {
    ...
    console.warn(
      `Song not found: ${modifiedSong.author} - ${modifiedSong.title}`,
    );
  }
  ...
  for (const condition of hardConditions) {
    ...
    if (result.tracks.items.length) return result.tracks.items;
    console.warn(
      `Song not found after HARD: ${hardSearch.author} - ${hardSearch.title}`,
    );
  }
...
export async function AddItemsToPlaylist(playlistId: string, uris: string[]) {
  ...
  if (uris.length > 100)
    throw new Error("Max 100 tracks per request, got " + uris.length);
