...
export async function wikiSearchExact(query: string) {
   const { data } = await axios.get(`https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exintro&explaintext&titles=${encodeURIComponent(query)}`);
...
export async function wikiSearchMusicianTitle(query: string) {
   const { data } = await axios.get(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(query)}`);
