...
export function groupArray(
  ...
  sortOrder: string,
  sortFn: string,
  combinedArray: SongDataQueried[],
) {
  let groupedArray;

  // Grouping
  if (groupFn === "year") {
    groupedArray = Object.groupBy(
      combinedArray,
      (q) => q.beatmapsetQuery.data?.submitted_date?.split("-")[0]!,
    );
  } else if (groupFn === "genre") {
    groupedArray = Object.groupBy(
      combinedArray,
      (q) => q.beatmapsetQuery.data?.genre.name!,
    );
    ...
    groupedArray = Object.groupBy(
      combinedArray,
      (q) => q.beatmapsetQuery.data?.artist!,
    );
  } else if (groupFn === "bpm") {
    groupedArray = Object.groupBy(combinedArray, (q) => {
  ...
  const sortedGroupedArray = Object.entries(groupedArray).reduce(
    (acc, [key, value]) => {
      const sortedArray = value.sort((a, b) => {
        const aData =
          sortOrder == "asc" ? a.beatmapsetQuery.data : b.beatmapsetQuery.data;
        const bData =
          sortOrder == "asc" ? b.beatmapsetQuery.data : a.beatmapsetQuery.data;
        ...
        switch (sortFn) {
          ...
          case "sort-date-mapped":
            return (
              new Date(aData.submitted_date).getTime() -
              new Date(bData.submitted_date).getTime()
            );
          case "sort-date-updated":
            return (
              new Date(aData.last_updated).getTime() -
              new Date(bData.last_updated).getTime()
            );
          // case 'sort-dif': return aData.difficulty - bData.difficulty;
          case "sort-length":
            return (
              aData.beatmaps[0].total_length - bData.beatmaps[0].total_length
            );
          case "sort-title":
