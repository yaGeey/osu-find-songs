...
export const sortBeatmapsMatrix = (
  a: BeatmapSet[],
  b: BeatmapSet[],
  sortFnName: string,
) => {
  ...
  switch (name) {
    ...
    case "difficulty":
      if (order === "asc")
        return (
          Math.min(
            ...a[0].beatmaps.map((beatmap) => beatmap.difficulty_rating),
          ) -
      ...
      if (order === "desc")
        return (
          Math.max(
            ...b[0].beatmaps.map((beatmap) => beatmap.difficulty_rating),
          ) -
    ...
    case "ranked":
      ...
      return (
        sign *
        (new Date(a[0].ranked_date).getTime() -
          new Date(b[0].ranked_date).getTime())
      );
    case "rating":
